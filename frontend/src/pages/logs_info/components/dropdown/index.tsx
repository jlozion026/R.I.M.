...
const DropDown: FC<IDropDown> = ({ report, setMenuTrig }) => {
  const { defaultUpdateData, updateForm2Data, setStartDate, setEndDate } = useContext(
    LogsInfoContext
  ) as LogsInfoContextType;

  useEffect(() => {
    if (report?.report?.report_type !== ReportType.CityProject) {
      setStartDate(new Date(report?.report?.incident?.date_started));
      setEndDate(new Date(report?.report?.incident?.date_ended));
      ...
      setStartDate(new Date(report.report.city_project?.date_started));
      setEndDate(new Date(report.report.city_project?.date_ended));
      defaultUpdateData["startDate"] = new Date(report?.report?.city_project?.date_started);
      defaultUpdateData["endDate"] = new Date(report?.report?.city_project?.date_ended);
